...
bool HalfEdgeMesh::AddFace(const std::vector<Vector3<float> > &verts) {
    ...
    size_t edge1, edge1P, edge2, edge2P, edge3, edge3P;
    AddHalfEdgePair(ind1, ind2, edge1, edge1P);
    AddHalfEdgePair(ind2, ind3, edge2, edge2P);
    AddHalfEdgePair(ind3, ind1, edge3, edge3P);
    // Connect inner ring
    ...
    e(edge1).face = mFaces.size() - 1;
    e(edge2).face = mFaces.size() - 1;
    e(edge3).face = mFaces.size() - 1;
...
std::vector<size_t> HalfEdgeMesh::FindNeighborVertices(size_t vertexIndex) const {
    ...
    currEdge = v(vertexIndex).edge;
    // Store start edge
    size_t startEdge = currEdge;

    do {
        // push pair vert to list
        oneRing.push_back(e(e(currEdge).pair).vert);
        // Get the new edge by getting prev and its pair
...
std::vector<size_t> HalfEdgeMesh::FindNeighborFaces(size_t vertexIndex) const {
    ...
    // Get start edge index
    currEdge = v(vertexIndex).edge;
    size_t startFace = e(currEdge).face;
    ...
    } while (e(currEdge).face != startFace);
...
float HalfEdgeMesh::VertexCurvature(size_t vertexIndex) const {
    ...
    for (size_t i = 0; i < oneRing.size(); i++) {
        ...
        angleSum += (Cotangent(prevPos, vj, vi) + Cotangent(nextPos, vj, vi)) * (vi - vj);

        // Area

        area += Cotangent(prevPos, vj, vi) + Cotangent(nextPos, vj, vi) * ((vi - vj) * (vi - vj));
    }

    return (angleSum / (4 * (area / 8))).Length();
}
...
Vector3<float> HalfEdgeMesh::VertexNormal(size_t vertexIndex) const {
    ...
    // Add your code here
    std::vector<size_t> oneRingFaces = FindNeighborFaces(vertexIndex);

    const Vector3<float> p1 = v(vertexIndex).pos;
    for (size_t i = 0; i < oneRingFaces.size(); i++) {

        n = n + FaceNormal(oneRingFaces[i]);
    }
    return n.Normalize();
}
...
float HalfEdgeMesh::Area() const {
    ...
    for (size_t i = 0; i < mFaces.size(); i++) {
        ...
        area += (1.0f / 2.0f) * Cross(e1, e2).Length();
    }
...
float HalfEdgeMesh::Volume() const {
    ...
    for (size_t i = 0; i < mFaces.size(); i++) {
        ...
        float area = (1.0f / 2.0f) * Cross(e1, e2).Length();

        // Normal
        Vector3<float> n = Cross(e1, e2).Normalize();

        // Centroid
        Vector3<float> centroid = (p1 + p2 + p3) / 3;

        // Volume
        volume += (centroid * n) * area / 3;
    }
...
int HalfEdgeMesh::Shells() const {
	...
	while (!que.empty())
	{
		//Get last and delete it
        size_t v = *que.begin();
        que.erase(v);
        tagged.insert(v);
        
		std::vector<size_t> vi = FindNeighborVertices(v);

		for (size_t i = 0; i < vi.size(); i++)
		{
			if (tagged.find(vi[i]) == tagged.end() && que.find(vi[i]) == que.end() )
			{
				que.insert(vi[i]);
			}
		}

		if (que.empty() && tagged.size() != mVerts.size())
		{
			counter++;
            que.insert(tagged.size());
...
size_t HalfEdgeMesh::Genus() const {
    ...
    size_t E = mEdges.size()/2;
    size_t F = mFaces.size();
    size_t L = F;
    size_t S = Shells();
    std::cout << V << " " << E << " " << F << " " << L << " " << S << std::endl;
	return (2*S - V + E -F +L -F) / 2;
}
